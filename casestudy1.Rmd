...
## * With the merged data above, we grouped by state, then did a count on the Brewery factor which gives us a total brewery count by state.
```{r question1}
...
## * We chose the mice library to address missing values in ABV and IBU. We imputed using all but the predm columns to create the new values.
```{r questin3}
...
```
---
title: "Question 5. Which state has the maximum alcoholic (ABV) beer? Which state has the most bitter (IBU) beer?"
output: html_document
runtime: shiny
---
...
```
---
title: "Question 6. Comment on the summary statistics and distribution of the ABV variable."
output: html_document
runtime: shiny
---
# **6. Comment on the summary statistics and distribution of the ABV variable.
## *The justABV variable holds only the ABV data.  That is used to plot the raw data histogram for Alcohol by volume for the entire population. The standard hist() was used to create a simple plot with a normal curve.
```{r}
...
```
---
title: "IBU Summary data and boxplot"
output: html_document
runtime: shiny
---
...
## *Jason's code to support usmap libary
```{r usmapplot}
...
# **1.cont US map of breweries by state.
...
## *Removed n/a's from the data merged data to one master data frame
```{r onemaster}
nona$State = as.character(nona$State)
nona$State = str_trim(nona$State)
...
## *This code creates a class for beer Styles to make analysis easier and to enable the plot dot plot used to describe the relationship between style and ABV and IBU
```{r beerstyle}
...
```
## *The dot plot of the relationship between Styles of beer and their ABV and IBU along with a measure of popularity by looking at total populations of the beer style classifications. This plot shows the popularity of IPA's along with their high ABV and IBU score.
```{r plotbeerstyle}
...
## *KNN was used to classify all ales into either IPA or Ale using ABV and IBU.  A training set and test set were created (80/20) and 60 values of k were used to determine the best k value for this model which is 7.
```{r question8knn}
library(stringr)
library(class)
library(caret)
...
```         
---
title: "Question 9. Display regions of the US and see if there is a trend to the type of beer and/or alcohol content. .northeast_region"
output: html_document
runtime: shiny
---
